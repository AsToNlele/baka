					...
					<div className="flex flex-col pt-8 gap-2 grow">
						{
							menuLinks.map((item) => {
								...
								if (isActive) {
									return (
										<Button as={Link} variant="flat" href={item.href} color="secondary" key={item.href}>
											{item.name}
										</Button>
									)
								}
								return (
									<Button as={Link} variant="light" href={item.href} key={item.href}>
										{item.name}
									</Button>
								)
							})
						...
						{!isLoading ? (
							<Dropdown
								...
								<DropdownMenu aria-label="User Actions" variant="flat" classNames={{
									...
									<DropdownItem key="profile" as={Link} href="/app/profile" className="text-foreground">
										Profile
									</DropdownItem>
