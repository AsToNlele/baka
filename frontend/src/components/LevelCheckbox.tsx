...
export const useLevelCheckbox = (
    locationOptions: LocationOptions,
    config?: LevelCheckboxConfig
) => {
    const [locations, setLocationsState] = useState<Array<string>>(
        config && config.defaultLocations
            ? Array.isArray(config.defaultLocations)
    ...
    const setLocations = (callback: (prev: Array<string>) => Array<string>) => {
        ...
        locationOptions.forEach((parent) => {
            if (parent.children) {
                ...
                let allChildrenChecked = tempLocations.filter((item) =>
                    item.includes(`${parent.value}-`)
                )
                allChildrenChecked = Array.from(new Set(allChildrenChecked))
                // If parent has all children checked, add parent
                if (
                    allChildrenChecked.length === allChildren.length &&
                    !tempLocations.includes(parent.value)
                ) {
                ...
                // If default option is set to Parent-*, add all children
                    ...
                    tempLocations.splice(
                        tempLocations.indexOf(`${parent.value}-*`),
                        1
                    )
                }
    ...
    const onChangeCapture = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (item.includes("-")) {
            ...
            if (
                locationOptions.find((parent) => parent.value === item)
                    ?.children
            ) {
                const children = allLocations.filter(
                    (child) => child && child.includes(item) && child !== item
                ) as Array<string>
                setLocations((prev) =>
                    isChecked
                        ? [...prev, ...children]
                        : prev.filter((prevItem) => !prevItem.includes(item))
                )
            } else {
                setLocations((prev) =>
                    isChecked
...
export const LevelCheckbox = ({
    ...
    controls: LevelCheckboxControls
}) => {
